...
export default function SubNavbar({ items, root }: SubNavbarProps) {
  ...
  return (
    <nav
      className={`cols bg-muted fixed z-30 grid h-10 w-full min-w-max grid-flow-col items-center border-red-700 text-sm ${cols} gap-2 px-2 md:flex md:flex-row md:pl-4`}
    >
      {items.map((item) => (
